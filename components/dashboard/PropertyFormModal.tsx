...
import { createProperty, updateProperty } from "@/lib/hooks";
...
const EXEMPTION_STATUSES: { value: ExemptionStatus; label: string }[] = [
...
export function PropertyFormModal({
  ...
  const [form, setForm] = useState<FormState>({
    ...
    exemptionType: property?.exemptionType ?? "wildlife",
    exemptionStatus: property?.exemptionStatus ?? "pending",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Number.isNaN(acreage) || acreage < 0) {
      setError("Enter a valid acreage.");
      return;
    }
    ...
    try {
      if (isEdit && property) {
        await updateProperty(property.slug ?? property.id, fields);
      } else {
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  set("exemptionType", e.target.value as ExemptionType)
                }
                className={inputClass}
              >
